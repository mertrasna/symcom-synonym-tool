...
class SynonymDeController extends Controller
{

    public function __construct(\App\User $user, \App\Admin $admin, \App\QuelleImportMaster $quelleImportMaster, \App\SynonymDe $synonymDe, \App\SynonymReference $synonymReference){
        $this->user = $user;
        $this->admin = $admin;
        $this->quelleImportMaster = $quelleImportMaster;
        $this->synonymDe = $synonymDe;
    ...
    /**
    * Fetching all SynonymDe Method 
    * Return : all SynonymDe 
    **/
    public function allSynonymDe(Request $request){
    	...
    	try{
	        ...
	        if($is_paginate == 0){
	        	$synonymDeData=$this->synonymDe
							->with('synonymreference')
	    					->orderBy('synonym_de.ersteller_datum', 'desc')
	    					->get();
	    		$dataArray['data']=$synonymDeData->toArray();
	        }else{
	        	$synonymDeData=$this->synonymDe
							->with('synonymreference')
	    					->orderBy('synonym_de.ersteller_datum', 'desc')
	    					->paginate($dataPerPage);
	    		$dataArray=$synonymDeData->toArray();
	        }
    ...
    **/
    public function addSynonymDe(Request $request)
	{ 
		$returnArr=config('constants.return_array');
		$input=$request->all();
		try{
			...
			// $returnArr['status']=2;
			// $returnArr['content']=$insertData;
			// $returnArr['message']="Synonym created successfully";
			// return $returnArr;
			// exit();
		    $synonymDeResult=$this->synonymDe->create($insertData);
		    if($synonymDeResult){
				$canProceed = 1;
				if(isset($insertData['synonym_reference_id']) AND !empty($insertData['synonym_reference_id'])){
					...
					$synonymReferenceInsertRes=DB::table('synonym_de_synonym_reference')->insert($synonymReferenceData);
				...
				if($canProceed == 1){
					$insertedData=$this->synonymDe->where('synonym_id', $synonymDeResult->id)->first();
	...
	/**
    * view SynonymDe Method
    * view SynonymDe information by it's ID
    * Return : a SynonymDe's informations
    **/
    public function viewSynonymDe(Request $request)
	{ 
		$returnArr=config('constants.return_array');
		$input=$request->all();
		try{
		    ...
		    $synonymDeData=$this->synonymDe->with('synonymreference')->where('synonym_id', $input['synonym_id'])->first();
	        if($synonymDeData === null){
	        	$returnArr['status']=4;
		        $returnArr['content']="";
		        $returnArr['message']="No synonym found with provided synonym_id";
	        }else{
	        	$result['data']=$synonymDeData;
	        	$returnArr['status']=2;
                $returnArr['content']=$result;
    ...
    **/
    public function updateSynonymDe(Request $request)
	{ 
		$returnArr=config('constants.return_array');
		$input=$request->all();
		try{
	        ...
	        $synonymData=$this->synonymDe->where('synonym_id', $input['synonym_id'])->first();
	        if($synonymData === null){
	        	$returnArr['status']=4;
		        $returnArr['content']="";
		        $returnArr['message']="No synonym found with provided synonym_id";
			    ...
			    $updateResult=$this->synonymDe->where('synonym_id', $input['synonym_id'])->update($updateData);

			    if($updateResult){
					$deleteExistingSynonymReference=DB::table('synonym_de_synonym_reference')->where('synonym_id', $input['synonym_id'])->delete();
					$canProceed = 1;
					...
					if($canProceed == 1){
						$fetchData=$this->synonymDe->where('synonym_id', $input['synonym_id'])->first();
						$returnArr['status']=2;
    ...
    **/
    public function deleteSynonymDe(Request $request)
	{ 
		$returnArr=config('constants.return_array');
		$input=$request->all();
		try{
    		...
    		{
				DB::table('synonym_de_synonym_reference')->whereIn('synonym_id', $input['synonym_id'])->delete();
    			$resultData=$this->synonymDe->whereIn('synonym_id', $input['synonym_id'])->delete();
	    		if($resultData){
		        	$returnArr['status']=2;
			        $returnArr['content']="";
		        ...
		        if($synonymData === null){
					...
					DB::table('synonym_de_synonym_reference')->whereIn('synonym_id', $input['synonym_id'])->delete();
		        	$resultData=$this->synonymDe->where('synonym_id', $input['synonym_id'])->delete();
			        if($resultData){
			        	$returnArr['status']=2;
				        $returnArr['content']="";
				        $returnArr['message']="Synonym deleted successfully";
