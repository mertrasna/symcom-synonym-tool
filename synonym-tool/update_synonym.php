...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update the database with new synonyms and root word
    $updateStmt = $db->prepare("UPDATE synonym_de SET root_word = ?, strict_synonym = ?, synonym_partial_1 = ?, synonym_general = ?, synonym_minor = ? WHERE LOWER(word) = LOWER(?)");
    $updateStmt->bind_param("ssssss", $rootWord, $strictSynonyms, $crossReferences, $hypernyms, $hyponyms, $word);

    if ($updateStmt->execute()) {
        error_log("Update Successful for: " . $word);
        echo json_encode(["success" => true, "message" => "Root word and synonyms updated successfully."]);
    } else {
