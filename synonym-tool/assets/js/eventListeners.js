...
$(document).ready(function () {
  ...
  function fetchSynonymsFromDictionary(selectedWord) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (synonymList.length === 0) {
          console.log(
            `ℹ️ No synonyms found for '${selectedWord}' on Dictionary.com.`
          );
          return;
        }
        ...
        console.log(
          "Sending data to insert_synonym.php (Dictionary.com):",
          synonymData
        );
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (response.success) {
              console.log(
                "✅ Successfully saved Dictionary.com synonyms for:",
                selectedWord
              );
            } else {
              console.warn(
                "⚠️ Error saving Dictionary.com synonyms:",
                response.message
              );
            }
          },
  ...
  function fetchSynonymsFromThesaurus(selectedWord) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (synonymList.length === 0) {
          console.log(
            `ℹ️ No synonyms found for '${selectedWord}' on Thesaurus.com.`
          );
          return;
        }
        ...
        console.log(
          "Sending data to insert_synonym.php (Thesaurus.com):",
          synonymData
        );
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (response.success) {
              console.log(
                "✅ Successfully saved Thesaurus.com synonyms for:",
                selectedWord
              );
            } else {
              console.warn(
                "⚠️ Error saving Thesaurus.com synonyms:",
                response.message
              );
            }
          },
  ...
  function addSynonymsToTable(word, synonyms) {
    if (!synonyms || synonyms.length === 0) {
      console.log("No synonyms to add to table");
      return; // ⛔ No synonyms provided, do nothing
    }

    // Check if table exists, if not create it
    if ($("#synonymTable").length === 0) {
      console.log("Creating synonym table as it doesn't exist");
      let tableHTML = `
      <div style="display: flex; align-items: center; justify-content: space-between; width: 100%; margin-bottom: 10px;">
          <p><b>Selected Word:</b> <span id="selected-word">${word}</span></p>
          <p><b>Root Word:</b> <input type="text" id="root-word" value="${word}" 
    ...
    if (newSynonyms.length === 0) {
      console.log("⚠️ No new synonyms to add (all were duplicates)");
      return; // ⛔ All synonyms were duplicates, do nothing
    }

    // Limit synonyms to 7
    if (newSynonyms.length > 7) {
      console.log(`ℹ️ Limiting from ${newSynonyms.length} to 7 synonyms`);
    ...
    let newRows = newSynonyms
      .map(
        (syn) => `
      <tr>
          <td><input type="checkbox" name="S" value="${syn}" checked></td>
  ...
  `
    ...
    try {
      ...
      if ($("#synonymTable tbody").length === 0) {
        console.log("⚠️ Table body not found, adding one");
        $("#synonymTable").append("<tbody></tbody>");
      }

      $("#synonymTable tbody").append(newRows);
      console.log(
        `✅ Successfully added ${newSynonyms.length} synonyms to the table!`
      );
    } catch (error) {
      console.error("❌ Error adding synonyms to table:", error);
    }
  }
