$(document).ready(function () {
    let $popup = $("<div id='popup-box' style='display:none; position:absolute; background-color: #fff; border: 1px solid #ccc; padding: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); transition: opacity 0.3s ease;'></div>");
    $("body").append($popup);

    let isLoading = false;
    let debounceTimer;

    // Function to debounce events
    function debounce(func, delay) {
        clearTimeout(debounceTimer);
        debounceTimer = setTimeout(func, delay);
    }

    // Show popup on hover
    $(document).on("mouseenter", ".synonym-word, .stopword", function (event) {
        let word = $(this).attr("data-word");
        console.log("Hovered Word:", word);

        // Prevent multiple AJAX calls if already loading
        if (isLoading) return;

        // Debounce to avoid firing the AJAX request too frequently
        debounce(function () {
            isLoading = true;
            // Show a loading message while waiting for the response
            $popup.text("Loading...").fadeIn(200);

            // Make an AJAX request to fetch word information from the PHP backend
            $.ajax({
                url: "fetch_word_info.php",
                type: "POST",
                data: { word: word },
                dataType: "json",
                success: function (response) {
                    if (response.success) {
                        let synonyms = response.synonyms;
                        let popupContent = "<strong> </strong><br><br>";

                        synonyms.forEach(function(info) {
                            popupContent += `
                                <strong>Word:</strong> ${info.word || 'N/A'}<br>
                                <strong>Root Word:</strong> ${info.root_word || 'N/A'}<br>
                                <strong>Strict Synonym:</strong> ${info.synonym || 'N/A'}<br>
                                <strong>Partial Synonym 1:</strong> ${info.cross_reference || 'N/A'}<br>
                                <strong>Partial Synonym 2:</strong> ${info.synonym_partial_2 || 'N/A'}<br>
                                <strong>General Synonym:</strong> ${info.generic_term || 'N/A'}<br>
                                <strong>Minor Synonym:</strong> ${info.sub_term || 'N/A'}<br>
                                <strong>Synonym NN:</strong> ${info.synonym_nn || 'N/A'}<br>
                                <strong>Comment:</strong> ${info.comment || 'N/A'}<br>
                                <strong>Status:</strong> ${info.non_secure_flag == 1 ? 'Active' : 'Inactive'}<br>
                                <strong>isgreen:</strong> ${info.isgreen == 1 ? 'Yes' : 'No'}<br>
                                <strong>Created At:</strong> ${info.created_at || 'N/A'}<br><br>
                            `;
                        });

                        $popup.html(popupContent);
                    } else {
                        $popup.html(`<span style="color: red;">${response.message}</span>`);
                    }
                    isLoading = false;
                },
                error: function (xhr, status, error) {
                    console.error("AJAX Error:", status, error);
                    $popup.html("Error fetching data. Please try again.");
                    isLoading = false;
                }
            });

            // Position the popup near the hovered word
            let offset = $(this).offset();
            let top = offset.top + $(this).outerHeight() + 5;
            let left = offset.left;

            $popup.css({ top: top, left: left }).fadeIn(200);
        }, 200); // Set a delay of 200ms for the debounce
    });

    // Hide popup when mouse leaves the word
    $(document).on("mouseleave", ".synonym-word, .stopword", function () {
        // Fade out the popup with a smooth transition
        $popup.fadeOut(200);
    });
});
